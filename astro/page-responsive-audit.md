...
2026-09-05 transparent logo header pass: removed the white logo plate, border, radius and card shadow from the shared header/footer brand container, then generated and referenced `/images/brand/ybd-global-logo-transparent.png` because the original logo file still contained a visible white background inside the bitmap. Desktop brand frame is now 218 x 58 with a 198 x 52 rendered logo and a subtle image-only drop shadow; mobile brand frame is 154 x 42. Astro check and production-domain builds to `dist-logo-check` and `dist-logo-transparent-check` passed. Browser DOM verification on `/` confirmed the transparent logo file is loaded, with transparent container background, no border, no container shadow and no horizontal overflow.

2026-09-05 dark-navigation logo contrast pass: generated `/images/brand/ybd-global-logo-dark-nav.png` from the transparent logo by preserving green brand elements and lifting the dark wordmark/linework to a light color for the black header and footer surfaces. Header and footer now reference the dark-nav asset instead of the low-contrast transparent logo. Astro check and production-domain build to `dist-logo-dark-nav-check` passed; browser DOM verification on `/` confirmed the dark-nav logo file is loaded, the brand container remains transparent and no horizontal overflow is present.

2026-09-05 dark-navigation logo final tuning: regenerated `/images/brand/ybd-global-logo-dark-nav.png` with a brighter wordmark/secondary text treatment and increased the shared header/footer logo frame from 218 x 58 to 238 x 62. The rendered desktop logo is now 218 x 56, while mobile uses a 168 x 44 frame with a 164 x 42 rendered logo. Astro check and production-domain build to `dist-logo-final-check` passed; browser verification on `/` confirmed the dark-nav logo loads and the page has no horizontal overflow.
...
| `/products/usb-cables/1m-5a-pd-type-c-to-type-c-cable/` | Product detail | Pass | Pass | Pass | New 1m 5A PD Type-C to Type-C cable detail page checked with supplied images, 100 pcs MOQ, US$0.28 / piece, 1000+ US$0.25 / piece, logo and custom packaging support. |
